    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        if (inputParagraphs > 1 && data.output[0]?.text) {
          const outputParagraphs = data.output[0].text
            .split(/\n\s*\n/)
            .filter((p) => p.trim().length > 0).length;
      ...
      <div className="flex w-full">
        <div className="w-full">
          ...
          <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
            <div>
              <Card className="border-border relative flex h-[calc(100vh-280px)] max-h-[600px] min-h-[400px] flex-col gap-0 overflow-visible rounded-none rounded-r-xl rounded-bl-xl border p-0">
            ...
            <div>
              {/* output */}
              <Card className="border-border relative h-[calc(100vh-280px)] max-h-[600px] min-h-[400px] overflow-y-auto border p-4">
